...
class DressingStEnv(AssistiveEnv):
    def __init__(self, robot, human):
        super(DressingStEnv, self).__init__(robot=robot, human=human, task='dressing', obs_robot_len=(17 + len(robot.controllable_joint_indices) - (len(robot.wheel_joint_indices) if robot.mobile else 0)), obs_human_len=(18 + len(human.controllable_joint_indices)))
        # self.tt = None
    ...
    def reset(self):
        super(DressingStEnv, self).reset()
        self.build_assistive_env()
        self.cloth_forces = np.zeros((1, 1))
        print(f"Wheelchair mounted: {self.robot.wheelchair_mounted}")
        ...
        joints_positions = [(self.human.j_right_elbow, -90), (self.human.j_left_shoulder_x, -45), (self.human.j_left_elbow, -90), (self.human.j_right_hip_x, 0), (self.human.j_right_knee, 0), (self.human.j_left_hip_x, 0), (self.human.j_left_knee, 0)]
        ...
        self.human.set_base_pos_orient([0, 0.03, 1.14 if self.human.gender == 'male' else 1.11], [0, 0, 0, 1])
        ...
        self.init_robot_pose(target_ee_pos, target_ee_orient, [(target_ee_pos, target_ee_orient)], [(shoulder_pos+offset, target_ee_orient_shoulder), (elbow_pos+offset, target_ee_orient), (wrist_pos+offset, target_ee_orient)], arm='left', tools=[], collision_objects=[self.human], right_side=False)
        if self.robot.mobile:
